...
def _safe_icon(path: str):
    # If icons are missing, fall back to a tiny blank icon (prevents crash)
    try:
...
def main():
    ...
    def monitor_loop():
        while True:
            ...
            try:
                icon_holder["icon"].icon = _safe_icon(ICON_RED if any_down else ICON_GREEN)
